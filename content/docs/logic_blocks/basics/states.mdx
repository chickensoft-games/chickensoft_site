...
For best results, don't add inputs during lifecycle callbacks. Instead, use them in handlers for services you're subscribed to, as shown below.
</Callout>
...
Here, `MyState`'s `OnDataReceived()` event handler uses the built-in `Input<T>()` method to add a `SomethingHappened` input to the logic block the state is attached to. `MyState` also implements the input handler for `SomethingHappened`, which allows us to return a new state — effectively forcing the logic block to change states when the `DataReceived` event occurs. This pattern is commonly leveraged with logic blocks to create complex reactive behaviors.
