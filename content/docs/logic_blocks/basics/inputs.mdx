---
title: 📥 Inputs
description: Making a logic block actually *do* something.
...
```csharp
public partial abstract record DimmableLightSwitchState : LogicBlockState {
...
<Callout type="tip">
By convention, inputs are stored in a simple static class called `Input` inside the logic block that owns them. This organizational pattern groups inputs and allows consistent referencing: e.g., `DimmableLightSwitchState.Input.DimmerUpdated`.

If that feels too verbose, you can always import the state statically with a `using static Project.DimmableLightSwitchState;` statement and then reference the nested types directly: `Input.DimmerUpdated`.
</Callout>
...
dimmableLightSwitch.Input(new DimmableLightSwitchState.Input.DimmerUpdated(0.5d));
```
