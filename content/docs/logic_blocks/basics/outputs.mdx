---
title: 📤 Outputs
description: Drive other systems indirectly.
...
```csharp
public abstract record DimmableLightSwitchState : LogicBlockState {
...
```csharp
public record PoweredOn : DimmableLightSwitchState {
  public PoweredOn() {
    this.OnEnter(() =>
...
- call methods directly on blackboard objects
  ...
  ```csharp
  public Type On(in Input.SomethingHappened input) {
