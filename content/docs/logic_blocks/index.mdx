---
title: 💡 LogicBlocks
description: Introduction to logic blocks.
...
- 🔵 [Logic Blocks Timer Tutorial](/docs/logic_blocks/tutorial)
...
LogicBlocks provides a source generator that can generate [UML state diagrams][UML] of your code. Add the `[StateDiagram]` attribute to your state base class to enable diagram generation.
...
- 🪢 OnStartSubscriptions callback

  Called when the logic block starts. Use it to register disposables tied to the logic block's lifecycle, which are automatically disposed along with the LogicBlock.
